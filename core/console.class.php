...
class Console {
    ...
    Static Public function load()
    {
        //error_reporting(0);
        //封装错误处理
        set_error_handler("Core\\Console::error");
        //封装异常处理
        set_exception_handler("Core\\Console::exception");
    }

    Static Public function error($errno, $errmsg, $filename, $linenum)
    {
        if(!in_array($errno, Conf::server('LOG','errortype')))return;
        $data = [
    ...
    {
        if(!in_array($exception->getCode(), Conf::server('LOG','errortype')))return;
        $data = [
            'datetime' => date("Y-m-d H:i:s"),
    ...
    Static Public function output($data)
    {
        ...
        if(Conf::server('LOG','errorfile') == 'xml')
            L(array2xml($data) . "\n\n", 'error', $mod_name);
        else
