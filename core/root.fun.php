...
{
    $keyt = $keyt ?? \Core\Conf::server('KEYT');
...
function session(string $key, $val = '[NULL]')
{
	switch ($key){
		case '[START]':
            ...
            U()->session = new \Core\Session($id);
			return false;
		case '[ID]':
			return U()->session->getId();
		case '[HAS]':
			if(empty($val) || $val === '[NULL]')return false;
			return \Core\Session::has($val);
		case '[CLEAR]':
            U('session')->save([]);
		default:
			if($val === '[NULL]'){
...
function C(string $key, $val = null)
{
    ...
    return call_user_func_array("\\Core\\Conf::get", $arr);
}
...
function M(string $tablename = null, string $dbname = null)
{
    if(empty($dbname)){
	    $dbname = 'DEFAULT';
	    if(\Core\Conf::database(U('dbname')))
            $dbname = U('dbname');
    }
    if(!\Core\Conf::database($dbname)){
        trigger_error("Database config \"{$dbname}\" is not exist!", E_USER_ERROR);
        return false;
    }
    $_model = new \Core\Base\Model($dbname);
...
function D(string $tablename, string $dbname = null)
{
    ...
    if(!isset(\Core\Root::$map[$class_name]) || !class_exists($class_name)){
...
function I($name, $default = false)
{
	...
	if(in_array($arr[0], ['get','post','cookie','server','files','header','request','input'])){
		$act = $arr[0];
		$data = \Core\Root::$user[getcid()]->$act;
...
{
    switch (\Core\Conf::server('LOG','split')){
    ...
    if(!is_file($filepath) && ($keep = \Core\Conf::server('LOG','keep')) > 0){
        $files = scandir($dir);
        $_files = [];
...
function T(string $tablename){
	...
	if(!isset($tables[$tablename])){
		$tables[$tablename] = new \Core\Table($tablename);
	}
...
function U(string $pname = null, $value = '[NULL]'){
    $uid = getcid();
    if(!isset(\Core\Root::$user[$uid]))return false;

    $user = \Core\Root::$user[$uid];
...
function Q(string $name)
{
    ...
    if(!isset($instances[$name])){
        $instances[$name] = new \Core\Util\Queue($name);
    }
