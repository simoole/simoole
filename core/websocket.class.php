...
abstract class Websocket
{
    ...
    Static Public function create(string $ip, int $port)
    {
        ...
        //设置websocket握手回调
        $serv->on('handshake', '\\Core\\Websocket::open');

        //设置websocket消息接收回调
        $serv->on('message', '\\Core\\Websocket::message');

        //设置websocket连接关闭回调
        $serv->on('close', '\\Core\\Websocket::close');
    ...
    Static Public function message(\swoole_server $serv, $frame)
    {
        ...
        if(Conf::server('TCP','is_binary')){
            if(strlen($frame_data) == 1){
                ...
                if(Conf::server('TCP','is_encrypt')){
                    $method = Conf::server('TCP','encrypt_func');
        ...
        if(strlen($frame_data) == 1){
            if($frame_data == 1){
                //收到手动心跳包，则将该数据包发送回去
                if(Conf::server('WEBSOCKET','heartbeat') == 2){
                    if(Conf::server('TCP','is_binary'))
    ...
    Static Public function heartbeat()
    {
        ...
        }elseif(Conf::server('WEBSOCKET','heartbeat') == 2){
